...
//        Toast.makeText(this,"NoteEditor_onCreat: \n getAction: "+getIntent().getAction(),Toast.LENGTH_SHORT).show();
        /*
...
//        Toast.makeText(this,"输出一下方法内的局部变量intent\n"+intent.getAction()
//                +"\n"+intent.getType()+"\n"
//                +intent.getData().toString(),Toast.LENGTH_LONG).show();

//        Toast.makeText(this,""+intent.getAction()
//                ,Toast.LENGTH_LONG).show();
//        Toast.makeText(this,""+intent.getType()
//                ,Toast.LENGTH_LONG).show();
//        Toast.makeText(this,""+intent.getData().toString()
//                ,Toast.LENGTH_LONG).show();
        ...
        if (Intent.ACTION_EDIT.equals(action)) {
            ...
            //如果是点击Inset或paste按钮进来的
        } else if (Intent.ACTION_INSERT.equals(action)
                || Intent.ACTION_PASTE.equals(action)) {

//            Toast.makeText(this,"点击了Paste或者Edit按钮\n",Toast.LENGTH_LONG).show();
            ...
            //状态都会变更为State_Insert
            mState = STATE_INSERT;
            //这里似乎只是将这个note本体的path返回回去
            //这里则是插入一条信息
            mUri = getContentResolver().insert(intent.getData(), null);
//            Toast.makeText(this,"NoteEditor_Insert: \n"+mUri.toString(),Toast.LENGTH_LONG).show();
...
//            Toast.makeText(this,""+mUri.toString()
//                    ,Toast.LENGTH_LONG).show();
         ...
         * 调用此命令可以设置活动将返回给调用者的结果。使用与触发Intent一起传入的URI，获取提供程序中的一个或多个注释。
         * 注意:这是在UI线程上完成的。它将阻塞线程，直到查询完成。
         * 在一个示例应用中，针对一个基于本地数据库的简单提供者，
         * 块将是暂时的，但在一个真正的应用中，你应该使用
         * android.content.AsyncQueryHandler或android.os.AsyncTask。
            从Build.VERSION_CODES开始。
            * 姜饼，你在这里提供的意图可以有意图。FLAG_GRANT_READ_URI_PERMISSION和/或Intent。
            * FLAG_GRANT_WRITE_URI_PERMISSION集。这将授予接收结果的Activity访问Intent中特定uri的权限。
            * 访问将一直存在，直到Activity结束(它将贯穿被杀死的宿主进程和其他临时销毁的进程)，
            * 并将被添加到它已经持有的任何现有URI权限集中。
         *
         */
